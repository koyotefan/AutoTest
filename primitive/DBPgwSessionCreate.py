...
class  DBPgwSessionCreate(TargetAgent):
    '''
    DB-PGW-세션-생성 primitive 에 대한 구현체 입니다.
    ...
    def __str__(self):
        return 'DBPgwSessionCreate'

    def make_request_data(self, _constant):
        # conf 에서 T_SESSION_INFO 의 Column 및 default 값을 config 에서 읽어 옵니다.
        # 읽은 값에 대고, _constant 가 그동안 축적한 값을 반영시킵니다.
        self.param_dict = _constant.get_template("DB_PGW_세션_테이블.conf")
    ...
    def _insert_sql(self, _param_dict, _db_dict):
        ...
        sql_list.append("INSERT INTO T_SESSION_INFO_{} (".format(_db_dict["TABLE_INDEX"]))
