...
class CdsSimStop(TargetAgent):
    '''
    CDS-SIM-STOP primitive 에 대한 구현체 입니다.
    '''
    ...
    def do(self, _req_dict):
        '''
        SnrAgent 에서는 /SmSimStop 에서 데이터를 받으면, do() 를 호출합니다.
        '''
        ...
        child = ChildProcess()
        if not child.init(get_sim_info("cds_sim")):
            res_dict["RESULT"] = "NOK"
            res_dict["REASON"] = "Unkown simualtor info for cds_sim"
            return res_dict

        if not child.stop():
            res_dict["RESULT"] = "NOK"
            res_dict["REASON"] = "simualtor down fail"
            return res_dict
